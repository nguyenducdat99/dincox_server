...
module.exports = {
  ...
  store: (req, res) => {
    try {
      let sql =
        "INSERT INTO " +
        "tblaccounts(user_name,password,position,email,join_at,status)" +
        " VALUE (?,?,?,?,?,?)";
      let data = req.body;
      let join_at = moment(new Date()).format("yyyy-MM-DD");

      db.query("SELECT * FROM tblaccounts", (err, response) => {
        if (err) throw err;
        ...
        response.forEach((element) => {
          if (
            element.user_name === data.user_name ||
  ...
  register: (req, res) => {
    try {
      let sql =
        "INSERT INTO " +
        "tblaccounts(user_name,password,position,email,address,join_at,status)" +
        " VALUE (?,?,?,?,?,?,?)";
      let data = req.body;
      let join_at = moment(new Date()).format("yyyy-MM-DD");
